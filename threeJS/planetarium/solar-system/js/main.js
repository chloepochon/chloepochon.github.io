...
camera.position.set(230, 100, -50);
// Focalisation sur le point central de la scène
camera.lookAt(0, 0, 0); 
...
// Positionnement et orientation des anneaux de Saturne
...
for (let i = 0; i < starCount; i++) {
  const x = Math.random() * 2000 - 1000; // Coordonnée x aléatoire dans la plage -2000 à 1000
  const y = Math.random() * 2000 - 1000; // Coordonnée y aléatoire dans la plage -2000 à 1000
  const z = Math.random() * 2000 - 1000; // Coordonnée z aléatoire dans la plage -2000 à 1000
...
function animate() {
  animationId = requestAnimationFrame(animate);
    ...
    // Faire tourner les planètes autour de leur axe Y (sur elles-mêmes)
...
function toggleVideoDemo() {
  if (isAnimationPlaying) {
    // Appeler la fonction d'arrêt de l'animation et de la musique
    stopAnimation();
    stopBackgroundMusic();
...
const minRotationSpeed = 0.0001; // Vitesse minimale de révolution 
const maxRotationSpeed = 0.1; // Vitesse maximale de révolution 
const defaultRotationSpeed = 0.01; // Vitesse de révolution par défaut
let rotationDirection = 1; // Direction de rotation (1 pour la rotation positive, -1 pour la rotation négative)
let currentRotationSpeed = defaultRotationSpeed; // Vitesse de révolution courante
...
$(document).ready(function() {
  ...
  $("#slider").slider({
    ...
    slide: function(event, ui) {
      // Mettre à jour la direction de rotation en fonction de la valeur du curseur
      rotationDirection = ui.value >= 0 ? 1 : -1;

      // Mettre à jour la vitesse de révolution courante avec la nouvelle vitesse absolue
...
function animateDefault() {
  ...
  // Augmenter l'animationTime en fonction de la direction de rotation et de la vitesse de révolution courante
...
function calculateRotationSpeed(value) {
  // Vérifier si la valeur du curseur est égale à 0, et attribuez la vitesse de révolution par défaut dans ce cas
  if (value === 0) {
    return 0; // La vitesse de rotation est nulle lorsque la valeur du curseur est à 0
  }

  // Calcul de la vitesse de révolution en fonction de la valeur du curseur
...
$(document).ready(function() {
    // Ajout de l'icône de planète à l'élément de curseur
